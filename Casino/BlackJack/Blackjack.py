...
while playing == "yes" or "y":
    ...
    while len(dealer_cards) != 2:                                                       # Generate dealer's first pair of cards and get sum of the cards
        ...
        def gen_dealer_card():                                                          # Creating the function to generate a random card
            ...
            dealer_cards.append(Cards.cards[int(ran)])
            dealer_cards_value.append(Cards.cards_value[int(ran)])
        gen_dealer_card()
    ...
    while len(player_cards) != 2:                                                       # Generate player's cards
        ...
        def gen_player_card():
            ran = random.uniform(0, 13)
            player_cards.append(Cards.cards[int(ran)])
            player_cards_value.append(Cards.cards_value[int(ran)])
        gen_player_card()
